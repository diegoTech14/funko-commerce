<?php 
    ...
    class Users extends dbAccess {
        ...
        private function authenticate($idUser, $password){
            $data = $this -> searchUser(idUser : $idUser);
        ...
        public function changePassword(Request $request, Response $response, $args){
            ...
            $body = json_decode($request -> getbody());
            $user = $this -> authenticate($args['id'], $body -> password);
        
            if($user){
                $data = $this -> editUserSession(idUser : $args['id'], passwordNew:Hash::hash($body -> passwordNew));
                $status = 200;
            }else{
                $status = 401;
            }
